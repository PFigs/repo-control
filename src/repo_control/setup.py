...
SYNC_STACK_LOCK = ".sync-stack.lock"
SYNC_STACK_SHIM = "sync-stack"


def sync_stack_lock_path(*, repo_path: Path) -> Path:
    """flock file guarding concurrent `sync-stack` runs for a repo."""
    return repo_path / HOOKS_DIR / SYNC_STACK_LOCK


def install_sync_stack_script(*, repo_path: Path, slug: str) -> None:
    """Write the per-repo `sync-stack` shim into <repo>/.repo-control/.

    A stable on-disk entry point for the flock-guarded restack of one repo.
    Idempotent — overwritten on every sync.
    """
    hooks = repo_path / HOOKS_DIR
    hooks.mkdir(parents=True, exist_ok=True)
    shim = hooks / SYNC_STACK_SHIM
    shim.write_text(
        "#!/usr/bin/env bash\n"
        "# Generated by repo-control. Flock-guarded `gt sync` from <repo>-main.\n"
        f'exec repo-control sync-stack "{slug}"\n'
    )
    shim.chmod(0o755)
